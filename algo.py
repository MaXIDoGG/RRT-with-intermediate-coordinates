...
class RRT(object):
    ...
    def __init__(self, start, goal, inters, obstacle_list, rand_area):
        """
        Setting Parameter
        ...
        self.inters = []
        for i in inters:
            self.inters.append(Node(i[0], i[1]))
        self.inters.append(self.end)
    ...
    def ShortestPath(self):
        if self.path == "Error":
            return "Error"
        m = self.Matrix()
        path = Dijkstra(m, 0, self.nodeList.index(self.inters[0]))
        for i in range(1, len(self.inters)):
            path = path + Dijkstra(m, path[-1],
                                   self.nodeList.index(self.inters[i]))
        if path == 'Error':
            return "Error"
        self.path = path
        return path
    ...
    def Planning(self):
        ...
        for i in self.inters:
            print(self.inters)
            min_index = self.Nearest([i.x, i.y])
            min_elem = self.nodeList[min_index]
            i.parent = min_index
            if self.CollisionFree([i.x, i.y], [min_elem.x, min_elem.y]):
...
def main(start, goal, inters, rand_area, obstacle_list):
    print("start RRT path planning")

    # Set Initial parameters
    rrt = RRT(start=start, goal=goal, inters=inters, rand_area=rand_area,
